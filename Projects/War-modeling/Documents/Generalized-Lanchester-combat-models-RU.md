...
**Примечание:** Содержание этого блокнота намеренно носит
...
-   Можно найти (относительно) хорошо отмеченные и пространные данные. Такие как:

    -   Размеры вооруженных сил

    -   Продолжительность боя
 
    -   Tочное время происхождения важных событий
...
-   Нам не потребуется:
    ...
    японские солдаты сражались до последнего
        ...
        -   Все японские содаты попавшие в плен находились там изключительно благодаря тому что их нашли без сознания.
...
-   $a$ и $e$ - нормa небоевых потерь

-   $b$ и $f$ - нормa потерь во время атак по площади

-   $c$ и $g$ - потери из-за прямого воздействия противника

-   $d$ и $h$ - размеры подкреплений или отступающих в резерв войск
...
-   Данное взаимодействие наиболее актуально, когда обе стороны расположены на общей территории:
    ...
    -   Т.е. это может быть партизанская война, репрессии, 
        вражда между двумя народами, война за контроль ограниченных ресурсов, и т.д.
...
-   Модель была вдохновлена вьетнамской войной и весьма точно и аккуратно ее описывает.

    -   Т.е. эта модель хорошо описывает ассиметричную войну,
...
интерполяции из двух точек данных - начальных и конечных значений количеств солдат каждой армии:
...
калибровочного расстояния мы получаем результаты, практически идеально совпадающими с теоретически рассчитаными показателями эффективности в \[JE1\]:
...
-   Создание интерактивных интерфейсов с UI для изменения параметров, на пример с возможностью выбора сценариев на основе известных сражений.
